 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t rep_map_char_add(ble_hids_t * p_hids, const ble_hids_init_t * p_hids_init)
{
    uint32_t            err_code;
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t inp_rep_characteristics_add(ble_hids_t            * p_hids,
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t outp_rep_characteristics_add(ble_hids_t            * p_hids,
                                             const ble_hids_init_t * p_hids_init)
{
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
uint32_t feature_rep_characteristics_add(ble_hids_t            * p_hids,
                                                const ble_hids_init_t * p_hids_init)
{
                               ...
                               uint8_t    * p_data)
{
    ...
    if (rep_index < p_hids->inp_rep_count)
    {
        ...
        if (p_hids->conn_handle != BLE_CONN_HANDLE_INVALID)
        {
			...
			NRF_LOG_INFO("err_code=%d, hvx_len=%d, len=%d\r\n", err_code, hvx_len, len);

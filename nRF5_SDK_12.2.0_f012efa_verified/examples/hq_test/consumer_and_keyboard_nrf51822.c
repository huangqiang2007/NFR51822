...
#define DEVICE_NAME                      "Moxi_Keyboard"                          /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "Moxi Ltd"                      /**< Manufacturer. Will be passed to Device Information Service. */
...
#define INPUT_REP_CNT					 2
#else
#define INPUT_REP_KEYBOARD_INDEX         0                                           /**< Index of Input Report. */
#define INPUT_REP_KEYBOARD_MAX_LEN       8                                           /**< Maximum length of the Input Report characteristic. */
#define INPUT_REP_KEYBOARD_ID            1                                           /**< Id of reference to Keyboard Input Report. */

#define INPUT_REP_MOUSE_INDEX 			 1
#define INPUT_REP_MOUSE_MAX_LEN 		 6
#define INPUT_REP_MOUSE_ID 			 	 2

#define INPUT_REP_ZOOM_INDEX			 2
#define INPUT_REP_ZOOM_MAX_LEN			 1
#define INPUT_REP_ZOOM_ID				 3

#define INPUT_REP_CNT					 3
#endif

#define OUTPUT_REP_KEYBOARD_INDEX        0                                           /**< Index of Output Report. */
#define OUTPUT_REP_KEYBOARD_MAX_LEN      1                                           /**< Maximum length of Output Report. */
#define OUTPUT_REP_KEYBOARD_ID           0                                           /**< Id of reference to Keyboard Output Report. */
#define OUTPUT_REPORT_BIT_MASK_CAPS_LOCK 0x02                                        /**< CAPS LOCK bit in Output Report (based on 'LED Page (0x08)' of the Universal Serial Bus HID Usage Tables). */


#define OUTPUT_REP_CNT					 1
...
static void timers_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	err_code = app_timer_create(&m_hid_button_timer_id,
                                APP_TIMER_MODE_SINGLE_SHOT,
                                hid_button_timeout_handler); 
    APP_ERROR_CHECK(err_code);
}
...
static void mouse_movement_send2(int16_t x_delta, int16_t y_delta, int8_t wheel)
{
    ...
    if (m_in_boot_mode) {
        ...
        uint8_t buffer[INPUT_REP_MOUSE_MAX_LEN] = {0};

        APP_ERROR_CHECK_BOOL(INPUT_REP_MOUSE_MAX_LEN == 6);
...
static void bsp_event_handler(bsp_event_t event)
{
    ...
    switch (event)
    {
		...
		case BSP_EVENT_KEY_1:			
			if (m_conn_handle != BLE_CONN_HANDLE_INVALID) {			
				mouse_media_send(MM_KEY_PLAY_PAUSE);
            }
            break;

        case BSP_EVENT_KEY_2:
			if (m_conn_handle != BLE_CONN_HANDLE_INVALID) {
				mouse_media_send(MM_KEY_VOL_DOWN);
            }
            break;

        case BSP_EVENT_KEY_3:
            if (m_conn_handle != BLE_CONN_HANDLE_INVALID) {       
				mouse_media_send(MM_KEY_VOL_UP);
            }
            break;
